...
//=========================================================================
void netStart() {
  wfs.println(F("AT"));           smartDelay(200);
  wfs.println(F("AT+CWJAP?"));    smartDelay(200);//SeeAP
  wfs.println(F("AT+CIFSR"));     smartDelay(200);//seesIP
  wfs.println(F("AT+CWMODE=1"));  smartDelay(200);//beWiFi
}
void netJoinAP(String s, String p) {
  wfs.println(F("+++"));          smartDelay(1000);
  //wfs.println(F("AT"));           smartDelay(1000);
  wfs.println(F("AT+CWMODE=1"));  smartDelay(1000);//beWiFi
  //wfs.println(F("AT+RST"));       smartDelay(2000);//reBoot
...
void HttpGetPost(String link, String text, int seconds) {
  int n = text.length() + 2;
  wfs.println(link); smartDelay(3000);
  wfs.print(F("AT+CIPSEND=")); wfs.println(n); smartDelay(2000); //wfs.println(prSend + String(n));smartDelay(1000);
