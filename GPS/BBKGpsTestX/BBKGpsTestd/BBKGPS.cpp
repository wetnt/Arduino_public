...
void BBKGPS::InitGPS(HardwareSerial &port,int baudrates,int TimeZones)
{
  //-----------------------------------------------------------------
  TimeZone = 8;
  lengGPRMC = 100;
...
bool BBKGPS::CheckHeadGPRMC()
{
  ...
  for(int i=0;i<6;i++){
    //-----------------------------------------
    //gpsport->print(charGPRMC[i]); 
    //gpsport->print("==");
    //gpsport->print(HeadGPRMC[i]);
    //gpsport->println();
    //-----------------------------------------
...
void BBKGPS::toStr(bool back)
{
  ...
  dtostrf(VV,4,2,str); sSped = str; 
  dtostrf(FF,4,2,str); sFans = str; 
  cLock = "V"; if(KK){ cLock = "A"; }
}
