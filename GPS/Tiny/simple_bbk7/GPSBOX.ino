...
void GpsTimeSet(){  
  //-------------------------------------------------------------------------
  setTime(HT,MT,ST,DD,MD,YY); mytime = now();
  sprintf(URL,"%.55s%.9s,%.10s,%s,%s,%s,%ld000",UseH,sLats,sLons,sHigh,sSped,sFans,mytime);
  //sprintf(URL,"%.55s%.9s,%.10s,%s,%s,%s,%d,%ld000",UseH,sLats,sLons,sHigh,sSped,sFans,SR,mytime);
  Serial.println(URL);
  //-------------------------------------------------------------------------
}
