...
void ReadIMUSensorHandle(void) {
  ...
  //  lg(accx/RAW_TO_G); lg(','); lg(imu.accRaw[0]); lg(','); lg(imu.accRaw[0]*RAW_TO_G/accx );lg(',');
  //  lg(gyrox/RAW_TO_DS); lg(','); lg(imu.gyroRaw[0]); lg(','); lg(imu.gyroRaw[0]*RAW_TO_DS/gyrox );lg();
  //lg(imu.accRaw[0]); lg(','); lg(imu.accRaw[1]); lg(','); lg(imu.accRaw[3]);
  //lg(imu.gyroRaw[0]); lg(','); lg(imu.gyroRaw[1]); lg(','); lg(imu.gyroRaw[3]); lg(','); lg(); //lg(',');
  //---------------------------------------------------------------------------------------------------
...
void IMU_show() {
  lg(imu.roll); lg(','); lg(imu.pitch); lg(','); lg(imu.yaw); 
    Serial.write(byte(10));
    //lg(','); lg(); //lg(',');
}
void IMU_loop() {
  MPU6050_read();//MPU6050_show();
  IMUSO3Thread(); IMU_show();
}
