...
void loop() {
  ...
  //----------------------------------------------------
  ax = ax + axoffs; ay = ay + ayoffs; az = az + azoffs;
  gx = gx + gxoffs; gy = gy + gyoffs; gz = gz + gzoffs;
  //calibration error
  //----------------------------------------------------
...
CEulerAngle fx =  GetEulerAngle(q0,q1,q2,q3);
...
void getoffs() {
  ...
  for (int i = 1; i <= n; i++) {
    accelgyro.getMotion6(&ax, &ay, &az, &gx, &gy, &gz);
    axsum = ax + axsum; aysum = ay + aysum; azsum = az + azsum - 16384; //去除重力方向?
