...
void LCD_Setup() {
  ...
  Serial.println("BBKLcd_Setup......OK!");
}
...
void LCD_Show_AVW(bool showkey){
  ...
  if(0){
    ...
    snprintf(lcd_b, 18, "%d %d %d.%02u %d.%02u", 
    Watt,Volt,
    (int)Amps,(long)(abs(Amps)*100+0.5) % 100,
    (int)Kwhs,(long)(abs(Kwhs)*100+0.5) % 100
      );
    //---------------------------------------
    LCD_Show(lcd_a,lcd_b,showkey);
    //---------------------------------------
  }
  else{
    //---------------------------------------
    a = ""; a.concat("W="); a.concat(Watt); a.concat(" "); a.concat("A="); a.concat(Watt);
    b = ""; b.concat("V="); b.concat(Volt); b.concat(" "); b.concat("K="); a.concat(Volt);
    c = ""; c.concat(time_hourss); c.concat(":"); c.concat(time_minute); c.concat(":"); c.concat(time_second);
    lcd.clear();
    lcd.setCursor(0,0);   
    lcd.print(c);
    lcd.setCursor(0,1);   
