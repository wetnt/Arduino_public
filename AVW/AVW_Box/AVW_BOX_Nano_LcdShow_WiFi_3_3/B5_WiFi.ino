...
void netStart() {
  wfs.listen();
  wfs.println(F("AT"));           smartDelayWifi(1000);
  wfs.println(F("AT+CWJAP?"));    smartDelayWifi(1000);//SeeAP
  wfs.println(F("AT+CIFSR"));     smartDelayWifi(1000);//seesIP
  wfs.println(F("AT+CWMODE=1"));  smartDelayWifi(1000);//beWiFi
}
void netJoinAP(String s, String p) {
  ...
  wfs.println("AT+CWJAP=\"" + s + "\",\"" + p + "\""); smartDelayWifi(10000);
  wfs.println(F("AT+CWJAP?"));    smartDelayWifi(1000);//SeeAP
  wfs.println(F("AT+CIFSR"));     smartDelayWifi(1000);//seesIP
}
//-------------------------------------------------------------------------
void HttpGetPost(String link, String text, int seconds) {  
  int n = text.length() + 2;
  wfs.listen();
  wfs.println(link); smartDelayWifi(2000);
  wfs.print(F("AT+CIPSEND=")); wfs.println(n); smartDelayWifi(1000); //wfs.println(prSend + String(n));smartDelay(1000);
  wfs.println(text); HttpBufferClear(); smartDelayWifi(seconds * 1000);
}
//=========================================================================
