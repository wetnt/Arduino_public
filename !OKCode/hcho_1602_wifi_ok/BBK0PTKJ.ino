...
//气体名称
String numName;
const char numNameInx[][6] = {//50% //String numNameInx[]= {//56%
...
//数据内容 单位
String numUnit;
const char numUnitInx[][6] = {"", "ppm", "VOL", "LEL", "Ppb", "Mg/m3"};
//数据当量
float numKeys = 1;
...
{
  //---------------------------------------
  lg(numName); lg(F(": ")); lg(fHCHO); lg(F(" ")); lgln(numUnit);
  //txt = String(numName) + String(F(": ")) + String(fHCHO) + String(F(" ")) + String(numUnit);
  //tst = String(dhtH) + String(F("C V:")) + String(fHCHOavr) + String(F(" ")) + String(numUnit);
  //lgln(txt);
  //lgln(tst);
  //---------------------------------------
}
