...
void Morse_Exp(char ch) {
  ...
  else if (ch == ' ') {  smartDelay(dotDelay * 7); return; }
  //------------------------------------------------------
  if (x.length() > 0) {
    lg(ch); lg('\t'); //lg(x); lg('\t');
    flashSequence(x);
...
void Morse_Run(boolean openKey) {
  digitalWrite(morse_blink_pin, openKey ? HIGH : LOW);
  digitalWrite(morse_blink_pi2, openKey ? HIGH : LOW);
  digitalWrite(morse_beeps_pin, openKey ? LOW  : HIGH);
}
//===================================================================
