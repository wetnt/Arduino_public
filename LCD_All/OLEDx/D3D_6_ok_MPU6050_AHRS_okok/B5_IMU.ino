...
//============================================================================
#define GYRO_CALC_TIME                        3000000l //us//校准时间
...
//============================================================================
void IMU_show() {
  lg(imu.q[0]); lg(','); lg(imu.q[1]); lg(','); lg(imu.q[2]);lg(','); lg(imu.q[3]);lg(',');
  lg(imu.roll); lg(','); lg(imu.pitch); lg(','); lg(imu.yaw); lg();
  //  //lg(',');
}
//============================================================================
float imu_dt = 0.01f;   //s
static uint32_t tPrev = 0; //us
float GetDT() {//! Time constant
  imu_dt = (tPrev > 0) ? (micros() - tPrev) / 1000000.0f : 0;
  tPrev = micros(); return imu_dt;
}
//===================================================================================================
void IMU_loop(void) {
  //-----------------------------------------------------------------------
...
void imu_read_physical(void) {//读取传感器，转换测量值为物理值
  //----------------------------------------------------------------------
  ...
  //----------------------------------------------------------------------
  for (int i = 0; i < 3; i++)  {//tutn to physical//测量值转换为物理值
...
void imu_filter(void) {//滤波
  ...
  imu.magb[0] = imu.magRaw[0];  imu.magb[1] = imu.magRaw[1];  imu.magb[2] = imu.magRaw[2];
  //---------------------------------------------------------------------------------------------------
...
boolean imu_offset_is_ok(void) {//初始校准//已经校准则
  if (imu.readys)
    return true;
  else {
...
void imu_offset() {
  //-------------------------------------------------------------
  for (int i = 0; i < 3; i++) {
    imu.gyroReal[i] = imu.gyro[i] - imu.gyroOffset[i];//rad/s
    imu.accbReal[i] = imu.accb[i]; //m/s^2
    imu.magbReal[i] = imu.magb[i]; //
  }
  //-------------------------------------------------------------
}
//===================================================================================================
//===================================================================================================
//===================================================================================================
//! Auxiliary variables to reduce number of repeated operations = 辅助变量减少重复计算
static float q0 = 1.0f, q1 = 0.0f, q2 = 0.0f, q3 = 0.0f;      /** quaternion of sensor frame relative to auxiliary frame */
static float dq0 = 0.0f, dq1 = 0.0f, dq2 = 0.0f, dq3 = 0.0f;  /** 四元数的传感器坐标系相对于副车架 */
static float gyro_bias[3] = {0.0f, 0.0f, 0.0f}; /** bias estimation=偏差估计*/
static float q0q0, q0q1, q0q2, q0q3, q1q1, q1q2, q1q3, q2q2, q2q3, q3q3; //辅助计算中间值
static boolean bFilterInit = false;//数值初始化标记
//===================================================================================================
...
void NonlinearSO3AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float twoKp, float twoKi, float dt) {
  ...
  if (!((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f))) {
    ...
    //-------------------------------------------------------------------------
    // Reference direction of Earth's magnetic field = 参考地球磁场的方向
  ...
  //-------------------------------------------------------------------------
  //获取四元数
  // Time derivative of quaternion. = 时间导数的四元数
