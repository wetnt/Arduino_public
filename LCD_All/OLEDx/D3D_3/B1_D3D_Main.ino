...
float iqRot(CQuaternion q, int v[], int result[]) {//融合qv
  float prod[4];
  prod[0] =  - q.y * v[0] - q.z * v[1] - q.w * v[2];
  prod[1] = q.x * v[0] + q.z * v[2] - q.w * v[1];
  prod[2] = q.x * v[1] - q.y * v[2] + q.w * v[0];
  prod[3] = q.x * v[2] + q.y * v[1] - q.z * v[0];
  result[0] = -prod[0] * q.y + prod[1] * q.x - prod[2] * q.w + prod[3] * q.z;
  result[1] = -prod[0] * q.z + prod[1] * q.w + prod[2] * q.x - prod[3] * q.y;
  result[2] = -prod[0] * q.w - prod[1] * q.z + prod[2] * q.y + prod[3] * q.x;
}
